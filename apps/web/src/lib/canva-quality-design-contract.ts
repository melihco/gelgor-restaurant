...
export function buildCanvaQualityDesignContract(input?: {
  ...
  return [
    ...
    'LOGO: Do NOT draw any brand mark. Keep one quiet corner calm for the post-production composite, background continuing as-is — paint no plate there, and never over letterforms or the photo hero.',
    colors,
    'TASTE FAIL (reject this look): amateur text overflow, letters cut by the frame edge, clipart sun/icons in place of the real mark, blank plate or outlined box awaiting a logo, Canva sandwich paint slabs, opaque brand-color side/diagonal panel ≥20% with white text on paint, flat opaque header ≥25% with centered white sans only, thin empty border + centered Times as the only craft, system-sans dump, cramped edge-hugging type, foreign scene invention.',
