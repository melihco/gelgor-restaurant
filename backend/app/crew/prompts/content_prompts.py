...
- "headline": REQUIRED for EVERY idea, including stories, reels, and pure_photo slots — never "" and never omitted. A `pure_photo` treatment describes how the IMAGE is made, not whether the piece has copy; a story still carries an overlay line. If you leave this empty the platform crops your caption to fit the canvas and paints a broken half-word. Campaign/marketing hook for ON-CANVAS design + feed display — max 60 characters. MUST be a punchy line derived from THIS idea's caption_draft message (same language as caption_draft). MUST NOT describe what is visible in a photo (no "sunset view", "cocktail on table", "interior shot"). Write the message first; the platform picks a gallery photo that fits this headline.
